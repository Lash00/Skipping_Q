...
export default function BankServiceTypeSelector({
  organization,
  branches,
  onSelectBranch,
  language,
    ...
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 style={{ color: "#36e27b" }} className="text-4xl font-bold mb-2">
          {language === "ar"
            ? `فروع ${getOrgName(organization)}`
            : `${getOrgName(organization)} Branches`}
        </h1>
        <p style={{ color: "#a8bbb3" }} className="text-lg">
          {language === "ar"
            ? "اختر الفرع لمشاهدة ماكينات الصراف الآلي المتاحة"
            : "Select a branch to view available ATMs"}
        </p>
      </div>
      ...
      <div className="mb-8 bg-gray-50 p-6 rounded-lg">
        ...
        <div className="flex items-center gap-4">
          ...
          <span
            style={{ color: "#111714" }}
            className="font-bold text-lg min-w-12"
          >
            {distanceFilter} km
          </span>
        </div>
      ...
      {branchesWithDistance.length === 0 ? (
        <div className="text-center py-12">
          <p style={{ color: "#a8bbb3" }} className="text-lg">
            {language === "ar"
              ? "لا توجد فروع في هذه المسافة"
              : "No branches in this distance range"}
          </p>
        </div>
      ) : (
